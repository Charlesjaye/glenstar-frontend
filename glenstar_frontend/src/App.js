...
export default function App() {
  ...
  const triggerScan = async () => {
    notify('Scanning all industrial report sources...', 'info');
    try {
      await fetch(`${API}/api/monitor/scan`, { method: 'POST' });
      const s = await fetch(`${API}/api/stats`).then(r => r.json());
      setStats(s);
      notify('Scan complete — reports updated.', 'success');
    } catch { notify('Scan failed — check connection.', 'error'); }
  };
  ...
  const pageMeta = {
    ...
    markets: { title: 'Market Analytics',         sub: 'Industrial fundamentals by building size across all 43 tracked US markets' },
    chat:    { title: 'Ask Claude',               sub: 'Conversational analysis grounded in 160+ industrial reports from 6 brokerages' },
    reports: { title: 'Report Library',           sub: `${stats?.report_count || 0} industrial reports from JLL, CBRE, C&W, Avison Young, Newmark, and Colliers` },
    monitor: { title: 'Source Monitor',           sub: 'Twice-daily scans at 6:00 AM and 6:00 PM — 6 brokerages · industrial only' },
  };
    ...
    <div className="shell">
        ...
        <div className="sidebar-footer">
          <div className="live-pill">
            ...
            <div>
              ...
              <div className="live-sub">{stats?.report_count || 0} reports · 6 brokerages</div>
            </div>
          </div>
        </div>
      </aside>
      ...
      <div className="main">
        <header className="topbar">
          ...
          <div className="tbr">
            <div className="tmeta">
              <div>{stats?.thesis_quarter || 'Q1 2026'} data</div>
              <div style={{color:'var(--dim)'}}>Industrial only</div>
            </div>
